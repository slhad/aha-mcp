...
|---------------|-----------|-------|-------------|---------------------|---------------------|
| 512 | `get-rest-automation-trace` | Get automation trace by rest_id and run_id | Fetches the trace for a specific automation run using its REST id and run id. | `rest_id` (string): Automation REST API id<br>`run_id` (string): Automation run id | None |
| 259 | `list-rest-automation-traces` | List automation traces by rest_id | List all traces for a specific automation using its REST id. | `rest_id` (string): Automation REST API id | None |
| 280 | `list-automations` | List all automations | List all automations | None | None |
| 68 | `delete-automation` | Delete an automation | Delete an automation by id | `id` (string): Automation unique ID | None |
| 337 | `get-automation-by-entity-id` | Get automation by entity_id | Find an automation entity using its entity_id (e.g. automation.my_automation). | `entity_id` (string): Automation entity_id, e.g. 'automation.my_automation' | None |
| 275 | `get-rest-automation-by-entity-id` | Get REST automation by entity_id | Get an automation's REST definition using its entity_id (e.g. automation.my_automation). | `entity_id` (string): Automation entity_id, e.g. 'automation.my_automation' | None |
| 228 | `update-automation-by-entity-id` | Update automation by entity_id | Update an automation using its entity_id (e.g. automation.my_automation). | `entity_id` (string): Automation entity_id, e.g. 'automation.my_automation'<br>`automation` (object): Home Assistant automation configuration object with ID, alias, description, triggers, conditions, actions, and execution mode | None |
| 327 | `get-automation-by-rest-id` | Get automation by rest_id | Find an automation entity using its REST API id (rest_id, not entity_id). | `rest_id` (string): Automation REST API id | None |
| 265 | `get-rest-automation-by-rest-id` | Get REST automation by rest_id | Get an automation's REST definition using its rest_id (REST API id, not entity_id). | `rest_id` (string): Automation REST API id | None |
...
| 87 | `list-device-automation-triggers` | List device automation triggers | List all triggers for a specific device's automations. | `device_id` (string): Device ID | None |
| 114 | `get-status` | Get Home Assistant connection status | Get Home Assistant connection status | None | None |
...
| 88 | `get-manifest` | Get Home Assistant integration manifest | Get the manifest of a Home Assistant integration | `integration` (string): Integration name, e.g. 'light' | None |
| 513 | `get-entity-registry-by-entity-id` | Get entity registry by entity_id | Get registry info for a specific entity_id | `entityId` (string): Entity ID, e.g. 'sensor.temperature' | None |
| 166 | `get-device-id-by-entity-id` | Get device_id by entity_id | Get the device_id for a given entity_id | `entityId` (string): Entity ID, e.g. 'sensor.temperature' | None |
| 167 | `get-config-entry-id-by-entity-id` | Get config_entry_id by entity_id | Get the config_entry_id for a given entity_id, useful for templated sensors and other config entry flows alike | `entityId` (string): Entity ID, e.g. 'sensor.temperature' | None |
...
| 97 | `create-config-entry-flow` | Create config entry flow with a handler helpers | Create a new config entry in the Home Assistant entity registry for a new config entry (new templated helpers for ex) | `handler` (string): The handler for the config entry flow | None |
...
| 108 | `create-config-entry-options-flow` | Create config entry options flow for a config entry id | Create a new config entry options flow for a specific config entry (already existing config entry, templated helpers for ex) | `config_entry_id` (string): The ID of the config entry to create options flow for | None |
...
| 116 | `list-config-entry-flows-helpers` | List helpers types | List available config entry flow handlers for helpers | None | None |
| 121 | `list-scripts` | List all scripts | List all scripts in Home Assistant, friendly_name = alias | None | None |
| 149 | `get-rest-script-by-entity-id` | Get a script by entity_id | Get the details of a specific script by its entity_id | `entityId` (string): Entity ID of the script, e.g. 'my_script' | None |
| 155 | `get-rest-script-by-alias` | Get a script by alias | Get the details of a specific script by its alias, not script.entity_id | `alias` (string): Alias of the script, e.g. 'My Script' not 'script.script_entity_id' | None |
| 156 | `update-rest-script-by-alias` | Update a script by alias | Update the details of a specific script by its alias | `alias` (string): alias of the script, e.g. 'My Script'<br>`data` (object): The script configuration data to update | None |
| 126 | `delete-rest-script-by-alias` | Delete a script by alias | Delete a specific script by its alias | `alias` (string): Alias of the script, e.g. 'My Script' | None |
| 91 | `list-entities-by-prefix` | List all entities by prefix | List all Home Assistant entities by prefix | `prefix` (string): Prefix to filter entities, e.g. 'sensor.' | None |
| 120 | `list-entities-by-regex` | List all entities by regex | List all Home Assistant entities matching a regex pattern | `pattern` (string): Regex pattern for entity IDs, e.g. '^sensor\.'<br>`flags` (string): Regex flags, e.g. 'i' for ignore case | None |
| 264 | `get-entity-state` | Get state of a specific entity | Get state of a specific entity_id | `entityId` (string): Entity ID, e.g. 'sensor.temperature' | None |
| 134 | `get-entity-domain` | Get domain of a specific entity | Get domain of a specific entity_id | `entityId` (string): Entity ID, e.g. 'sensor.temperature' | None |
| 139 | `search-related` | Search related entities | Search for entities related to a specific item_type (like 'area') and entity_id (like 'studio') | `itemType` (string): Type of the item, e.g. 'entity', 'area'<br>`itemId` (string): ID of the item, e.g. 'sensor.temperature' or 'area.living_room' | None |
| 40 | `list-area` | List all areas | List all Home Assistant areas | None | None |
...
| 49 | `list-lovelace-dashboards` | List Lovelace Dashboards | List all Lovelace dashboards. | None | None |
...
| 72 | `delete-lovelace-dashboard` | Delete Lovelace Dashboard | Delete a Lovelace dashboard by dashboard_id. | `dashboard_id` (string):  | None |
| 46 | `get-lovelace-resources` | Get Lovelace Resources | Fetch Lovelace resources. | None | None |
...
### Get a script by entity_id

**Name:** `get-rest-script-by-entity-id`

**Description:** Get the details of a specific script by its entity_id

**Required Parameters:**
- `entityId` (string): Entity ID of the script, e.g. 'my_script'

---

### Get a script by alias

**Name:** `get-rest-script-by-alias`

**Description:** Get the details of a specific script by its alias, not script.entity_id

**Required Parameters:**
- `alias` (string): Alias of the script, e.g. 'My Script' not 'script.script_entity_id'

---

### Update a script by alias

**Name:** `update-rest-script-by-alias`

**Description:** Update the details of a specific script by its alias

**Required Parameters:**
- `alias` (string): alias of the script, e.g. 'My Script'
- `data` (object): The script configuration data to update

---

### Delete a script by alias

**Name:** `delete-rest-script-by-alias`
