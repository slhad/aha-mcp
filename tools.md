...
| 178 | `validate-config` | Validate triggers, conditions and actions for any automation change | Validate triggers, conditions and actions configurations as if part of an automation. Any changes to automation should be checked with this tool | `config` (object): {"type":"object","properties":{"triggers":{"type":"array","items":{}},"conditions":{"type":"array","items":{}},"actions":{"type":"array","items":{}}},"additionalProperties":false} | None |
...
| 82 | `list-entities-by-prefix` | List all entities by prefix | List all Home Assistant entities by prefix | `prefix` (string): Prefix to filter entities, e.g. 'sensor.' | None |
...
| 154 | `get-lovelace-config` | Get Lovelace Config | Fetch Lovelace dashboard config by url_path. The URL must contain a dash ('-') and must not contain spaces or special characters, except '_' and '-' | `url_path` (string): The URL path of the Lovelace dashboard config to fetch, e.g., 'default-view' or 'dashboard-id' 
